...
for i = 1:n_robots
    ...
    eval(['g' num2str(i) ' = ' meas_model_str(i, n_robots, n_landmarks)]);
...
function str = meas_model_str(i, n_robots, n_landmarks)
str = '[';
j = 1;
while j <= n_robots
    if (j ~= i)
        str = [str 'norm(p' num2str(i) ' - p' num2str(j) '); atan2(RR' ...
            num2str(i) '(2,:)*(p' num2str(j) '-p' num2str(i) '), RR' ...
            num2str(i) '(1,:)*(p' num2str(j) '-p' num2str(i) '));'];
    end
    j = j + 1;
end
for j = 1:n_landmarks
